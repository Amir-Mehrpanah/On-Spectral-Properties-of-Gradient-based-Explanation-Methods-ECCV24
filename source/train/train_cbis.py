...
def rescale_image(x, size):
  ...
  return tf.image.resize(x, shape, method='bicubic')
...
def train_and_evaluate(config):
    ...
    rot = tf.keras.layers.RandomRotation(25./360.)
    ...
    def train_prerocess(x):
        x = rescale_image(x, size=config.img_size) # This is needed as some images have a dim smaller than img_size

        #x = tf.image.resize_with_crop_or_pad(x, config.img_size, config.img_size)

        # Data augmentation was used on-the-fly by doing the following random transformations: horizontal and vertical flips, rotation in
        # [-25, 25] degrees, shear in [-0.2, 0.2] radians, zoom in [0.8, 1.2] ratio and channel shift in [-20, 20] pixel values.
        
        # x = tf.numpy_function(lambda x: tf.keras.preprocessing.image.random_shear(x,  0.2 * (np.pi / 180.0)), [x], tf.float32)
        # x = tf.numpy_function(lambda x: tf.keras.preprocessing.image.random_zoom(x, [0.8, 1.2], row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest'), [x], tf.float32)
        # x = tf.numpy_function(lambda x: tf.keras.preprocessing.image.random_channel_shift(x,  20), [x], tf.float32)
        
        #x = rot(x, training=True)
        ##x = keras_cv.layers.RandomShear(0.2 * (np.pi / 180.0), 0.2 * (np.pi / 180.0))(x)
        #x = zoom(x, training=True)
        ##x = keras_cv.layers.RandomChannelShift([0, 255], factor, channels=1)(x)
    ...
    def val_prerocess(x):
        x = rescale_image(x, size=config.img_size)
        #x = tf.keras.layers.CenterCrop(config.img_size, config.img_size)(x)
        #x = tf.image.resize_with_crop_or_pad(x, config.img_size, config.img_size)
    ...
    ds_train, ds_info = tfds.load(name='curated_breast_imaging_ddsm', split='train', with_info=True, data_dir=config.data_dir)
    ds_val = tfds.load(name='curated_breast_imaging_ddsm', split='validation', with_info=False, data_dir=config.data_dir)
    ...
    if config.arch == 'resnet18':
        model = fm.ResNet18(output='log_softmax', pretrained=None, num_classes=config.num_classes, dtype=dtype, normalize=False)
    elif config.arch == 'resnet34':
        model = fm.ResNet34(output='log_softmax', pretrained=None, num_classes=config.num_classes, dtype=dtype, normalize=False)
    elif config.arch == 'resnet50':
        model = fm.ResNet50(output='log_softmax', pretrained=None, num_classes=config.num_classes, dtype=dtype, normalize=False)
    elif config.arch == 'resnet101':
        model = fm.ResNet101(output='log_softmax', pretrained=None, num_classes=config.num_classes, dtype=dtype, normalize=False)
    elif config.arch == 'resnet152':
        model = fm.ResNet152(output='log_softmax', pretrained=None, num_classes=config.num_classes, dtype=dtype, normalize=False)
    ...
    learning_rate_fn = optax.piecewise_constant_schedule(init_value=config.learning_rate,
                                                         boundaries_and_scales={ 5 * steps_per_epoch: 1.0,
